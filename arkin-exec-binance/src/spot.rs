...
use crate::config::BinanceSpotExecutionConfig;
use crate::types::{BinanceCancelResponse, BinanceOrderResponse, OrderParams};

type HmacSha256 = Hmac<Sha256>;

#[async_trait::async_trait]
pub trait BinanceSpotRequest {
...
impl BinanceSpotClient {
    ...
    pub async fn execute_request<R: serde::de::DeserializeOwned + Send + 'static>(
        &self,
        request: &impl BinanceSpotRequest,
    ) -> Result<R, Box<dyn std::error::Error + Send + Sync>> {
        let url = self.base_url.join(&format!("api/v3{}", request.endpoint()))?;
        ...
        debug!("Executing spot request to: {} with method: {}", url.as_str(), request.method());
        ...
        if !response.status().is_success() {
            ...
            tracing::error!("Binance Spot API error: {} - {}", status, body);
    ...
    pub async fn cancel_all_orders(
        ...
        let _response: Vec<crate::types::BinanceCancelResponse> = self.execute_request(&request).await?;
        Ok(())
    }
}

// Request implementations for Spot
pub struct PlaceOrderRequest {
    pub params: OrderParams,
}

#[async_trait::async_trait]
impl BinanceSpotRequest for PlaceOrderRequest {
    fn endpoint(&self) -> &str {
        "/order"
    }
...
#[async_trait::async_trait]
impl BinanceSpotRequest for CancelOrderRequest {
    fn endpoint(&self) -> &str {
        "/order"
    }
...
#[async_trait::async_trait]
impl BinanceSpotRequest for CancelAllOrdersRequest {
    fn endpoint(&self) -> &str {
        "/openOrders"
    }

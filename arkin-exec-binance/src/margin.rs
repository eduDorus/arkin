...
impl BinanceMarginClient {
    ...
    pub async fn execute_request<R: serde::de::DeserializeOwned + Send + 'static>(
        &self,
        request: &impl BinanceMarginRequest,
    ) -> Result<R, Box<dyn std::error::Error + Send + Sync>> {
        let url = self.base_url.join(&format!("sapi/v1{}", request.endpoint()))?;
        ...
        debug!(
            "Executing margin request to: {} with method: {}",
            url.as_str(),
            request.method()
        );
        ...
        if !response.status().is_success() {
            ...
            tracing::error!("Binance Margin API error: {} - {}", status, body);
...
#[async_trait::async_trait]
impl BinanceMarginRequest for PlaceOrderRequest {
    fn endpoint(&self) -> &str {
        "/order"
    }
...
#[async_trait::async_trait]
impl BinanceMarginRequest for CancelOrderRequest {
    fn endpoint(&self) -> &str {
        "/order"
    }
...
#[async_trait::async_trait]
impl BinanceMarginRequest for CancelAllOrdersRequest {
    fn endpoint(&self) -> &str {
        "/openOrders"
    }

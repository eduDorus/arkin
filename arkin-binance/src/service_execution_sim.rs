...
impl SimulationExecutor {
    pub fn new(venue: Arc<Venue>, init_balance: HashMap<Arc<Asset>, Decimal>) -> Arc<Self> {
        let account = InternalAccount::default();
        for (asset, balance) in init_balance {
            account.update_balance(&asset, balance);
        }
        Self {
            venue,
    ...
    async fn place_order(&self, ctx: Arc<CoreCtx>, order: &VenueOrder) {
        ...
        ctx.publish(Event::VenueOrderInflight(order.clone().into())).await;
        ...
        ctx.publish(Event::VenueOrderPlaced(order.into())).await;
    }

    async fn cancel_order(&self, ctx: Arc<CoreCtx>, order: &VenueOrder) {
        ...
        if let Some((_, order)) = self.orderbook.remove(order.id) {
            ...
            ctx.publish(Event::VenueOrderCancelled(order.into())).await;
        } else {
            warn!(target: "executor-simulation", "order {} not in order book, could not cancel", order.id);
        }
    }

    async fn cancel_all(&self, ctx: Arc<CoreCtx>) {
    ...
    async fn fill_update(
        ...
        // Add the fill
        let mut order = order.clone();
        order.add_fill(ctx.now().await, price, quantity, commission);
        ...
        ctx.publish(Event::VenueOrderFill(order.to_owned().into())).await;
    ...
    async fn tick_update(&self, ctx: Arc<CoreCtx>, tick: &Tick) {
        ...
        for order in self.orderbook.list_orders() {
            if order.instrument.id == tick.instrument.id {
                ...
                if matched {
                    self.fill_update(ctx.clone(), &order, price, quantity, commission).await;
                    self.trade_update(ctx.clone(), &order, price, quantity, commission).await;
                }
            }
        }
    }
...
impl Runnable for SimulationExecutor {
    async fn handle_event(&self, ctx: Arc<CoreCtx>, event: Event) {
        match &event {
            ...
            Event::CancelAllVenueOrders(_) => self.cancel_all(ctx).await,
    ...
    async fn setup(&self, _service_ctx: Arc<ServiceCtx>, core_ctx: Arc<CoreCtx>) {
        ...
        core_ctx.publish(Event::InitialAccountUpdate(au.to_owned().into())).await;

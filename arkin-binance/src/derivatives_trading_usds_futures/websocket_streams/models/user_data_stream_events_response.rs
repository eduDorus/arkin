    ...
    fn try_from(v: Value) -> Result<Self, Self::Error> {
        ...
        match tag {
            "ACCOUNT_CONFIG_UPDATE" => {
                ...
                Ok(UserDataStreamEventsResponse::AccountConfigUpdate(Box::new(payload)))
            }

            "ACCOUNT_UPDATE" => {
                let payload = serde_json::from_value(v)?;
                Ok(UserDataStreamEventsResponse::AccountUpdate(Box::new(payload)))
            }

            "CONDITIONAL_ORDER_TRIGGER_REJECT" => {
                let payload = serde_json::from_value(v)?;
                Ok(UserDataStreamEventsResponse::ConditionalOrderTriggerReject(Box::new(payload)))
            }
            ...
            "ORDER_TRADE_UPDATE" => {
                let payload = serde_json::from_value(v)?;
                Ok(UserDataStreamEventsResponse::OrderTradeUpdate(Box::new(payload)))
            }

            "STRATEGY_UPDATE" => {
                let payload = serde_json::from_value(v)?;
                Ok(UserDataStreamEventsResponse::StrategyUpdate(Box::new(payload)))
            }
            ...
            "listenKeyExpired" => {
                let payload = serde_json::from_value(v)?;
                Ok(UserDataStreamEventsResponse::ListenKeyExpired(Box::new(payload)))
            }

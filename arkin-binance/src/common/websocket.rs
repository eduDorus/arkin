...
impl WebsocketCommon {
    ...
    async fn init_connect(
        ...
        self.on_open(url.to_string(), conn.clone(), old_writer).await;
...
impl WebsocketApi {
        ...
        R: DeserializeOwned + Send + Sync + 'static,
    {
        ...
        if options.with_api_key || options.is_signed {
            params.insert(
                "apiKey".into(),
                Value::String(self.configuration.api_key.clone().expect("API key must be set")),
            );
        }
        if options.is_signed {
            let ts = get_timestamp();
            let ts_i64 = i64::try_from(ts).map_err(|e| WebsocketError::Protocol(e.to_string()))?;
            params.insert("timestamp".into(), Value::Number(serde_json::Number::from(ts_i64)));
        ...
        let maybe_rx = self
            .common
            .send(serde_json::to_string(&request).unwrap(), Some(id.clone()), true, timeout, None)
            .await?;
...
// #[cfg(test)]
// mod tests {
//     use crate::common::config::{ConfigurationWebsocketApi, ConfigurationWebsocketStreams, PrivateKey};
//     use crate::common::errors::WebsocketError;
//     use crate::common::models::TimeUnit;
//     use crate::common::utils::{build_user_agent, SignatureGenerator};
//     use crate::common::websocket::{
//         create_stream_handler, PendingRequest, ReconnectEntry, WebsocketApi, WebsocketBase, WebsocketCommon,
//         WebsocketConnection, WebsocketEvent, WebsocketEventEmitter, WebsocketHandler, WebsocketMessageSendOptions,
//         WebsocketMode, WebsocketStream, WebsocketStreams,
//     };
//     use async_trait::async_trait;
...
//                     assert!(matches!(result, Err(crate::errors::WebsocketError::NotConnected)));
//                 });
//             }

...
mod tests {
    ...
    async fn test_large_batch_unsorted_commit() {
        ...
        let all_values = state.last_n(Some(instrument.clone()), feature_id.clone(), now, 1000);
        ...
        let first = state.last(Some(instrument.clone()), feature_id.clone(), now - Duration::from_secs(1000));
        ...
        let middle = state.last(Some(instrument.clone()), feature_id.clone(), now - Duration::from_secs(500));
        assert_eq!(middle, Some(500.0));

        // Verify lag works correctly
        assert_eq!(state.lag(Some(instrument.clone()), feature_id.clone(), now, 0), Some(999.0));
        assert_eq!(state.lag(Some(instrument.clone()), feature_id.clone(), now, 1), Some(998.0));
        assert_eq!(state.lag(Some(instrument.clone()), feature_id.clone(), now, 10), Some(989.0));
        assert_eq!(state.lag(Some(instrument.clone()), feature_id.clone(), now, 999), Some(0.0));

        // Verify window works correctly
        let window_end = now - Duration::from_secs(100);
        let window_values = state.window(Some(instrument), feature_id, window_end, Duration::from_secs(10));
    ...
    async fn test_multiple_features_large_unsorted_batch() {
        ...
        for (inst, feat, offset) in [
            ...
            let values = state.last_n(Some(inst.clone()), feat.clone(), now, 250);
    ...
    fn test_periods() {
        ...
        // the last 3 => 12, 13, 14
        let p = state.last_n(Some(instrument), feature_id, now, 3);
    ...
    async fn test_wal_buffer_and_commit() {
        ...
        let intervals = state.last_n(Some(instrument.clone()), feature_id.clone(), now, 3);
        assert_eq!(intervals, vec![2.0, 3.0, 4.0]);
    }
    ...
    async fn test_ttl_expiration() {
        ...
        let intervals = state.last_n(Some(instrument), feature_id, now, 10);
    ...
    async fn test_commit_with_multiple_instruments_and_features() {
        ...
        let last = state.last(Some(instrument1.clone()), feature_id1.clone(), now);
        assert_eq!(last, Some(6.0));
        let intervals = state.last_n(Some(instrument1.clone()), feature_id1.clone(), now, 10);
        assert_eq!(intervals, vec![4.0, 1.0, 6.0]); // Sorted by time

        // Verify instrument2, feature1 has values [7.0, 2.0] sorted by time
        let intervals = state.last_n(Some(instrument2.clone()), feature_id1.clone(), now, 10);
        assert_eq!(intervals, vec![7.0, 2.0]); // Sorted by time

        // Verify instrument1, feature2 has value [3.0]
        let last = state.last(Some(instrument1.clone()), feature_id2.clone(), now);
        assert_eq!(last, Some(3.0));

        // Verify instrument2, feature2 has value [5.0]
        let last = state.last(Some(instrument2), feature_id2, now);

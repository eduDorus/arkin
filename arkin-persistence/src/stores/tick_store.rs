...
pub async fn stream_range_buffered(
    ...
    let stream = stream::iter(time_chunks)
        .map(move |(start_batch, end_batch)| {
            ...
            async move {
                info!(target: "persistence", "Streaming ticks for batch: {} - {}", start_batch, end_batch);

                // Get the cursor for this time chunk
                let mut cursor =
                    match tick_repo::stream_range(&ctx_clone, &instrument_ids, start_batch, end_batch).await {
                        Ok(c) => c,
                        Err(e) => {
                            ...
                            Ok(Some(dto)) => {
                                ...
                                if let Some(instrument) = local_instrument_lookup.get(&dto.instrument_id) {
                                    let trade = Tick::builder()
                                        ...
                                        .ask_quantity(dto.ask_quantity)
                                        .build();
                                    yield Event::TickUpdate(Arc::new(trade));
                                } else {
...
//         async move {
//             info!(target: "persistence", "Fetching ticks for batch: {} - {}", start_batch, end_batch);
...
//                     .ask_quantity(dto.ask_quantity)
//                     .build();
//                 ticks.push(Event::TickUpdate(Arc::new(tick)));
//             }
//             ticks
//         }
//     });

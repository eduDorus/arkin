#[cfg(test)]
mod tests {
    use crate::WideQuoterExecutionStrategy;
    ...
    async fn test_wide_quoter_reentry() {
        ...
        let mut venue_order_1_cancelled = venue_order_1.clone();
        venue_order_1_cancelled.cancel(time.now().await);
        venue_order_1_cancelled.finalize_terminate(time.now().await);
        exec_strategy
            .handle_event(core_ctx.clone(), Event::VenueOrderCancelled(venue_order_1_cancelled.into()))
            .await;

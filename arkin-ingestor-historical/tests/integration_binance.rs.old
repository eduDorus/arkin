...
#[tokio::test]
#[test_log::test]
async fn test_integration_binance_spot_trades() {
    info!("Testing Binance Spot Trades integration");
    ...
    while let Some((ts, json)) = stream.next().await {
        ...
        info!("Binance Spot Event: {}: {:?}", ts, parsed);
        event_count += 1;
        if event_count >= 5 {
...
async fn test_integration_binance_spot_agg_trades() {
    info!("Testing Binance Spot AggTrades integration");
    ...
    while let Some((_ts, json)) = stream.next().await {
        ...
        match stream_event.data {
            ExchangeEventData::Tick(tick) => {
                ...
                info!(
                    "Parsed Binance Spot Book Ticker: bid={}/{}, ask={}/{}",

...
mod tests {
    ...
    use crate::{
        ...
        derivatives_trading_usds_futures::{
            websocket_api::{NewOrderParams, NewOrderSideEnum, NewOrderTimeInForceEnum, StartUserDataStreamParams},
    ...
    async fn subscribe_user_stream() -> Result<()> {
        ...
        // Subscribe to the stream
        let _stream = connection.subscribe_on_ws_events(|e| println!("{:?}", e));

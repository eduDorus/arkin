...
use super::config::{ConfigurationRestApi, ConfigurationWebsocketApi, HttpAgent, PrivateKey, ProxyConfig};
...
pub(crate) static ID_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^[0-9a-f]{32}$").unwrap());
...
impl SignatureGenerator {
            ...
            match pk {
                PrivateKey::File(path) => {
                    if Path::new(path).exists() {
                        fs::read_to_string(path).with_context(|| format!("Failed to read private key file: {path}"))
                    } else {
    ...
    fn get_key_object(&self) -> Result<&PKey<openssl::pkey::Private>> {
        self.key_object.get_or_try_init(|| {
            ...
            if let Some(pass) = self.private_key_passphrase.as_ref() {
                ...
                PKey::private_key_from_pem(key_data.as_bytes()).context("Failed to parse private key")
            }
        })
    }
    ...
    fn get_ed25519_signing_key(&self, key_obj: &PKey<openssl::pkey::Private>) -> Result<&SigningKey> {
        self.ed25519_signing_key.get_or_try_init(|| {
            let der = key_obj.private_key_to_der().context("Failed to export Ed25519 key to DER")?;
            SigningKey::from_pkcs8_der(&der).map_err(|e| anyhow::anyhow!("Failed to parse Ed25519 key: {}", e))
        })
    }
    ...
    pub fn get_signature(&self, query_params: &BTreeMap<String, Value>) -> Result<String> {
        ...
        if self.private_key.is_some() {
            ...
            match key_obj.id() {
                openssl::pkey::Id::RSA => {
                    let mut signer =
                        OpenSslSigner::new(MessageDigest::sha256(), key_obj).context("Failed to create RSA signer")?;
                    signer.update(params.as_bytes()).context("Failed to update RSA signer")?;
...
///
#[must_use]
pub fn build_client(timeout: u64, keep_alive: bool, proxy: Option<&ProxyConfig>, agent: Option<HttpAgent>) -> Client {
    ...
    if let Some(proxy_conf) = proxy {
        let protocol = proxy_conf.protocol.clone().unwrap_or_else(|| "http".to_string());
...
pub fn validate_time_unit(time_unit: &str) -> Result<Option<&str>, anyhow::Error> {
    match time_unit {
        ...
        _ => Err(anyhow::anyhow!("time_unit must be either 'MILLISECOND' or 'MICROSECOND'")),
    }
}
    ...
    for (key, value) in params {
        ...
        let value_str = match value {
            ...
            Value::Array(_) | Value::Object(_) => {
                serde_json::to_string(value).with_context(|| format!("failed to JSON-serialize `{}`", key))?
            }
...
#[must_use]
pub fn should_retry_request(error: &reqwest::Error, method: Option<&str>, retries_left: Option<usize>) -> bool {
    let method = method.unwrap_or("");
    let is_retriable_method = method.eq_ignore_ascii_case("GET") || method.eq_ignore_ascii_case("DELETE");
    ...
    loop {
        ...
        match client.execute(req_clone).await {
            Ok(response) => {
                ...
                if status.is_client_error() || status.is_server_error() {
                    let error_msg = serde_json::from_str::<serde_json::Value>(&content)
                        .ok()
                        .and_then(|v| v.get("msg").and_then(|m| m.as_str()).map(std::string::ToString::to_string))
            ...
            Err(e) => {
                ...
                return Err(ConnectorError::ConnectorClientError(format!("HTTP request failed: {e}")));
            }
        }
    }
}
...
///
pub fn replace_websocket_streams_placeholders<V, S>(input: &str, variables: &HashMap<&str, V, S>) -> String
where
    V: Display,
    S: BuildHasher,
{
...
//                 assert!(res.is_err(), "expected an error (timeout or connect) but got {res:?}");
//                 let elapsed = start.elapsed();
//                 assert!(elapsed < Duration::from_millis(500), "timed out too slowly: {elapsed:?}");
//             });
//         }
...
//             assert!(user_agent.starts_with(&expected_prefix), "prefix mismatch: {user_agent}");

//             assert!(user_agent.contains(rustc), "user agent missing RUSTC_VERSION: {user_agent}");

//             assert!(user_agent.contains(&format!("; {os}")), "user agent missing OS: {user_agent}");
//             assert!(
...
//             assert_eq!(user_agent1, user_agent2, "user agent should be the same on repeated calls");
//         }
//     }
...
//             assert!(ts >= before, "timestamp {ts} is before captured before time {before}");
//             assert!(ts <= after, "timestamp {ts} is after captured after time {after}");
//         }
...
//             assert!(t2 >= t1, "second timestamp {t2} is not >= first timestamp {t1}");
//         }
//     }
...
//             let params = mk_map(vec![("foo", json!("bar")), ("num", json!(42)), ("flag", json!(true))]);
//             let qs = build_query_string(&params)?;
...
//             let params = mk_map(vec![("space", json!("hello world")), ("symbols", json!("a/b?c"))]);
//             let qs = build_query_string(&params)?;
//             let mut parts = vec![];
//             let mut ser = Serializer::new(String::new());
...
//             let expected = Serializer::new(String::new()).append_pair("foo", &json).finish();
//             assert_eq!(qs, expected);
//         }
...
//             let expected = Serializer::new(String::new()).append_pair("mix", &json).finish();
//             assert_eq!(qs, expected);
//         }
...
//             let expected = Serializer::new(String::new()).append_pair("objs", &json).finish();
//             assert_eq!(qs, expected);
//         }
...
//             let json = serde_json::to_string(&json!({})).unwrap();
//             let expected = Serializer::new(String::new()).append_pair("emp", &json).finish();
//             assert_eq!(qs, expected);
//         }
...
//             let params = mk_map(vec![("こんにちは", json!("世界")), ("weird key/?=", json!("val"))]);
//             let qs = build_query_string(&params).unwrap();
...
//             let expected = Serializer::new(String::new()).append_pair("a", &json).finish();
//             assert_eq!(qs, expected);
//         }
...
//             let expected = Serializer::new(String::new()).append_pair("a=b&c%", "val").finish();
//             assert_eq!(qs, expected);
//         }
...
//             let sig = signature_gen.get_signature(&params).expect("HMAC signing failed");
...
//             let signature_gen = SignatureGenerator::new(None, Some(PrivateKey::Raw(priv_pem.clone())), None);
//             let sig = signature_gen.get_signature(&params).expect("RSA signing failed");
...
//             let rsa = Rsa::generate(2048).unwrap();
//             let priv_pem = rsa.private_key_to_pem().unwrap();
//             let signature_gen = SignatureGenerator::new(None, Some(PrivateKey::Raw(priv_pem)), None);
//             let s1 = signature_gen.get_signature(&params).unwrap();
//             let s2 = signature_gen.get_signature(&params).unwrap();
//             assert_eq!(s1, s2);
//         }
...
//             let signature_gen = SignatureGenerator::new(None, Some(PrivateKey::Raw(priv_pem.clone())), None);
//             let sig = signature_gen.get_signature(&params).expect("Ed25519 signing failed");

//             let pem_str = String::from_utf8(priv_pem).unwrap();
//             let b64 = pem_str.lines().filter(|l| !l.starts_with("-----")).collect::<String>();
...
//             let err = signature_gen.get_signature(&params).unwrap_err().to_string();
...
//             let signature_gen = SignatureGenerator::new(None, Some(PrivateKey::Raw(b"not a key".to_vec())), None);
//             let err = signature_gen.get_signature(&params).unwrap_err().to_string();
...
//             let resp = Response::from(http::response::Response::builder().status(code).body("").unwrap());
...
//         #[test]
//         fn multiple_headers() {
//             let headers = mk_headers(vec![("X-MBX-USED-WEIGHT-1h", "10"), ("x-mbx-order-count-2d", "20")]);
...
//             let headers = mk_headers(vec![("x-mbx-used-weight-3x", "5"), ("random-header", "100")]);
//             let limits = parse_rate_limit_headers(&headers);
...
//         use crate::{
//             common::utils::http_request, config::ConfigurationRestApi, errors::ConnectorError, models::RestApiResponse,
//         };
...
//                 let result = http_request::<Dummy>(req, &cfg).await;
//                 assert!(matches!(result, Err(ConnectorError::TooManyRequestsError(_))));
...
//                 let result = http_request::<Dummy>(req, &cfg).await;
//                 assert!(matches!(result, Err(ConnectorError::ConnectorClientError(_))));
//                 mock.assert();
//             });
//         }
...
//                 let result =
//                     send_request::<TestResponse>(&configuration, "/api/v1/test", Method::GET, params, None, false)
//                         .await?;
...
//                 let result =
//                     send_request::<TestResponse>(&configuration, "/api/v3/order", Method::POST, params, None, true)
//                         .await?;
...
//                 let result =
//                     send_request::<TestResponse>(&configuration, "/api/v1/data", Method::GET, params, None, false)
//                         .await?;
...
//                 let result =
//                     send_request::<TestResponse>(&configuration, "http://invalid", Method::GET, params, None, false)
//                         .await;
...
//                 server.mock(|when, then| {
//                     when.method(GET).path("/api/v1/test").header("X-MBX-TIME-UNIT", "MILLISECOND");
//                     then.status(200)
//                         .header("content-type", "application/json")
...
//                 server.mock(|when, then| {
//                     when.method(GET).path("/api/v1/test").header("X-My-Test", "all-clear");
//                     then.status(200)
//                         .header("content-type", "application/json")
//                         .body(r#"{"message":"ok"}"#);
//                 });
...
//                 server.mock(|when, then| {
//                     when.method(GET).path("/api/v1/test").header("X-Good", "safe");
//                     then.status(200)
//                         .header("content-type", "application/json")
...
//         fn length_is_32() {
//             let s = random_string();
//             assert_eq!(s.len(), 32, "random_string() should be 32 chars, got {}", s.len());
//         }
...
//             assert_eq!(bytes.len(), 16, "hex::decode returned {} bytes", bytes.len());
//         }
...
//             let a = random_string();
//             let b = random_string();
//             assert_ne!(a, b, "Two calls to random_string() returned the same value: {a}");
//         }
//     }
...
//             assert_eq!(result.len(), 1, "expected only one entry, got {}", result.len());
...
//             assert_eq!(sorted_keys, [&"a".to_string(), &"b".to_string(), &"c".to_string()]);
//         }
//     }
...
//             );
//             assert!(sorted.contains_key("new"), "Sorted map should reflect its own insertions");
//         }
//     }
...
//             assert_eq!(replace_websocket_streams_placeholders("/<symbol>", &vars), "btc");
//         }
...
//             let out = replace_websocket_streams_placeholders("/<symbol>@depth<levels>@<updateSpeed>", &vars);
...
//             assert_eq!(replace_websocket_streams_placeholders("<My_Stream-Key>", &vars), "Value");
//         }
...
//             assert_eq!(replace_websocket_streams_placeholders(input, &vars), "pre-ABC-post");
//         }
//     }
...
//             assert!(!cleaned.contains_key("b"), "Empty string should be stripped");
//             assert!(cleaned.contains_key("c"), "Non-empty string should be kept");

//             let cfg = make_config();
//             let (_id, req) =
//                 build_websocket_api_message(&cfg, "method", payload, &WebsocketMessageSendOptions::default(), true);
//             let params = &req["params"];

...
use super::{
    ...
    utils::{build_websocket_api_message, random_string, validate_time_unit, ID_REGEX},
};
...
pub trait WebsocketHandler: Send + Sync + 'static {
    ...
    async fn get_reconnect_url(&self, default_url: String, connection: Arc<WebsocketConnection>) -> String;
}
...
impl WebsocketCommon {
    ...
    async fn get_available_connections(&self, allow_non_established: bool) -> Vec<Arc<WebsocketConnection>> {
    ...
    async fn get_connection(&self, allow_non_established: bool) -> Result<Arc<WebsocketConnection>, WebsocketError> {
    ...
    async fn get_reconnect_url(&self, default_url: &str, connection: Arc<WebsocketConnection>) -> String {
        if let Some(handler) = {
            let conn_state = connection.state.lock().await;
...

        {
            ...
            if conn_state.renewal_pending {
                ...
                if let Some(tx) = old_ws_writer {
                    ...
                    let _ = self.close_connection_gracefully(tx, Arc::clone(&connection)).await;
                }
                return;
            }
    ...
    async fn create_websocket(
        ...
        match timeout(timeout_duration, handshake).await {
            ...
            Err(_) => {
                error!("WebSocket connection timed out after {}s", timeout_duration.as_secs());
            ...
            spawn(async move {
                ...
                while let Some(msg) = rx.recv().await {
                    if let Err(e) = sink.send(msg).await {
                        let failure_reason = WebsocketConnectionFailureReason::from_tungstenite_error(&e);

                        error!("Write error on {}: {:?}, classified as {:?}", wconn.id, e, failure_reason);
                        ...
                        if !conn_state.close_initiated && !is_renewal && failure_reason.should_reconnect() {
                            ...
                            conn_state.is_session_logged_on = false;
                            drop(conn_state);
                            let reconnect_url = common_clone.get_reconnect_url(&writer_url, Arc::clone(&wconn)).await;
...

        {
            ...
            spawn(async move {
                while let Some(item) = read_half.next().await {
                    match item {
                        Ok(Message::Text(msg)) => {
                            common.on_message(msg.to_string(), Arc::clone(&reader_conn)).await;
                        }
                        Ok(Message::Binary(bin)) => {
                            ...
                            common.on_message(decompressed, Arc::clone(&reader_conn)).await;
                        }
                        ...
                        Ok(Message::Close(frame)) => {
                            ...
                            let failure_reason =
                                WebsocketConnectionFailureReason::from_close_code(code, user_initiated);
    ...
    async fn send(
        ...
        let ws_write_tx = {
            let conn_state = conn.state.lock().await;
            conn_state.ws_write_tx.clone().ok_or(WebsocketError::NotConnected)?
        };
...
impl WebsocketApi {
    ...
    pub fn new(configuration: ConfigurationWebsocketApi, connection_pool: Vec<Arc<WebsocketConnection>>) -> Arc<Self> {
        ...
        let common = WebsocketCommon::new(
            ...
            usize::try_from(configuration.reconnect_delay).expect("reconnect_delay should fit in usize"),
            agent_clone,
        ...
        R: DeserializeOwned + Send + Sync + 'static,
    {
        ...
        let do_multi = options.is_session_logon.unwrap_or(false) || options.is_session_logout.unwrap_or(false);
        ...
        let (id, request) = build_websocket_api_message(&self.configuration, method, payload, &options, skip_auth);
        ...
        for connection in &connections {
            let opt_rx = self
                .common
                .send(raw_payload.clone(), Some(id.clone()), true, timeout, Some(connection.clone()))
                .await?;
...
impl WebsocketHandler for WebsocketApi {
    ...
    async fn on_open(&self, _url: String, connection: Arc<WebsocketConnection>) {
        ...
        if already_logged_on {
            debug!("Connection {} already logged on, skipping re-logon", connection.id);
            return;
        }
        ...
        spawn(async move {
            let (id, json_msg) = build_websocket_api_message(&configuration, &method, payload, &options, false);

            let raw_message = match serde_json::to_string(&json_msg) {
                Ok(msg) => msg,
                Err(e) => {
                    warn!("Failed to serialize session logon message for connection {}: {}", conn.id, e);
                    return;
                }
            };

            debug!("Session re-logon on connection {}: {}", conn.id, raw_message);
                ...
                .await
            {
                Ok(Some(rx)) => rx,
                Ok(None) => {
                    warn!("Session re-logon dispatch returned None for connection {}", conn.id);
                    return;
                }
                Err(e) => {
                    warn!("Session re-logon dispatch failed on connection {}: {}", conn.id, e);
                    return;
                }
            };
            ...
            let final_result = match result {
                ...
                Err(e) => {
                    warn!("Session re-logon receiver error on connection {}: {}", conn.id, e);
                    return;
                }
            };
            ...
            let payload = match final_result {
                Ok(payload) => payload,
                Err(e) => {
                    warn!("Session re-logon payload error on connection {}: {}", conn.id, e);
                    return;
                }
            };

            debug!("Session re-logon succeeded on connection {}: {}", conn.id, payload);
            let mut conn_state = conn.state.lock().await;
    ...
    async fn get_reconnect_url(&self, default_url: String, _connection: Arc<WebsocketConnection>) -> String {
        default_url
    }
}
...
impl WebsocketStreams {
    ...
    pub async fn subscribe(self: Arc<Self>, streams: Vec<String>, id: Option<String>) {
        let streams: Vec<String> = {
            ...
            streams.into_iter().filter(|s| !map.contains_key(s)).collect()
        };
        ...
        for (conn, streams) in connection_streams {
            if !self.common.is_connected(Some(&conn)).await {
                info!("Connection is not ready. Queuing subscription for streams: {:?}", streams);
                let mut conn_state = conn.state.lock().await;
    ...
    pub async fn unsubscribe(&self, streams: Vec<String>, id: Option<String>) {
        let request_id = id.filter(|s| ID_REGEX.is_match(s)).unwrap_or_else(random_string);
        ...
        for stream in streams {
            ...
            let conn = if let Some(c) = maybe_conn {
                if !self.common.is_connected(Some(&c)).await {
                    warn!("Stream {} not associated with an active connection.", stream);
                    continue;
                }
                c
            } else {
            ...
            let callbacks = {
                let conn_state = conn.state.lock().await;
                conn_state.stream_callbacks.get(&stream).is_none_or(std::vec::Vec::is_empty)
            };
            ...
            spawn(async move {
                let _ = common.send(msg, None, false, Duration::ZERO, Some(conn_clone)).await;
            });
    ...
    async fn handle_stream_assignment(&self, streams: Vec<String>) -> Vec<(Arc<WebsocketConnection>, Vec<String>)> {
        ...
        for stream in streams {
            ...
            if need_new {
                match self.common.get_connection(true).await {
                    ...
                    Err(err) => {
                        warn!("No available WebSocket connection to subscribe stream `{}`: {:?}", stream, err);
                        continue;
                    }
                }
            }
            ...
            if let Some(conn) = conn_opt {
                {
                    let mut conn_state = conn.state.lock().await;
                    conn_state.stream_callbacks.entry(stream.clone()).or_default();
                }
...
impl WebsocketHandler for WebsocketStreams {
    ...
    async fn on_open(&self, _url: String, connection: Arc<WebsocketConnection>) {
        let pending_subs: Vec<String> = {
            let mut conn_state = connection.state.lock().await;
            take(&mut conn_state.pending_subscriptions).into_iter().collect()
        };
    ...
    async fn on_message(&self, data: String, connection: Arc<WebsocketConnection>) {
        let msg: Value = match serde_json::from_str(&data) {
            Ok(v) => v,
            Err(err) => {
                error!("Failed to parse WebSocket stream message {} – {}", data, err);
                return;
            }
        };

        let (stream_name, payload) = match (msg.get("stream").and_then(Value::as_str), msg.get("data").cloned()) {
        ...
        let callbacks = {
            let conn_state = connection.state.lock().await;
            conn_state.stream_callbacks.get(&stream_name).cloned().unwrap_or_else(Vec::new)
        };
    ...
    async fn get_reconnect_url(&self, _default_url: String, connection: Arc<WebsocketConnection>) -> String {
    ...
    T: DeserializeOwned + Send + 'static,
{
        ...
        F: Fn(T) + Send + Sync + 'static,
    {
        ...
        let cb_wrapper: Arc<dyn Fn(&Value) + Send + Sync> =
            Arc::new(move |v: &Value| match serde_json::from_value::<T>(v.clone()) {
        ...
        match &self.websocket_base {
            WebsocketBase::WebsocketStreams(ws_streams) => {
                let conn = {
                    ...
                    map.get(&self.stream_or_id).cloned().expect("stream must be subscribed")
                };

                {
                    let mut conn_state = conn.state.lock().await;
                    let entry = conn_state.stream_callbacks.entry(self.stream_or_id.clone()).or_default();

                    if !entry.iter().any(|existing| Arc::ptr_eq(existing, &cb_wrapper)) {
            ...
            WebsocketBase::WebsocketApi(ws_api) => {
                ...
                let entry = stream_callbacks.entry(self.stream_or_id.clone()).or_default();
        ...
        if let Some(cb) = maybe_cb {
            match &self.websocket_base {
                WebsocketBase::WebsocketStreams(ws_streams) => {
                    let conn = {
                        let map = ws_streams.connection_streams.lock().await;
                        map.get(&self.stream_or_id).cloned().expect("stream must have been subscribed")
                    };
    ...
    T: DeserializeOwned + Send + 'static,
{
    match &websocket_base {
        WebsocketBase::WebsocketStreams(ws_streams) => {
            ws_streams.clone().subscribe(vec![stream_or_id.clone()], id.clone()).await;
        }
...
// #[cfg(test)]
// mod tests {
//     use crate::common::utils::{build_user_agent, SignatureGenerator};
//     use crate::common::websocket::{
//         create_stream_handler, PendingRequest, ReconnectEntry, SendWebsocketMessageResult, WebsocketApi, WebsocketBase,
//         WebsocketCommon, WebsocketConnection, WebsocketEvent, WebsocketEventEmitter, WebsocketHandler,
//         WebsocketMessageSendOptions, WebsocketMode, WebsocketSessionLogonReq, WebsocketStream, WebsocketStreams,
//     };
...
//         oneshot, Mutex,
//     };
//     use tokio::time::{advance, pause, resume, sleep, timeout, Duration};
...
//         let sig_gen = SignatureGenerator::new(Some("api_secret".into()), None::<PrivateKey>, None::<String>);
...
//                 let received = timeout(Duration::from_millis(100), rx).await.expect("timed out");
...
//             fn pool_mode() {
//                 TOKIO_SHARED_RT.block_on(async {
//                     let common = WebsocketCommon::new(vec![], WebsocketMode::Pool(3), 0, None, None);
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 10, None, None);
...
//                     let conn = WebsocketConnection::new("c1");
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 5, None, None);
//                     let url = format!("ws://{addr}");
//                     common
//                         .reconnect_tx
//                         .send(ReconnectEntry {
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 200, None, None);
//                     let url = format!("ws://{addr}");
//                     common
//                         .reconnect_tx
//                         .send(ReconnectEntry {
...
//                 let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                 let url = "wss://example".to_string();
//                 common.renewal_tx.send((conn.id.clone(), url)).await.unwrap();
...
//                 let conn = WebsocketConnection::new("c1");
//                 let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                 common.renewal_tx.send(("other".into(), "u".into())).await.unwrap();
//                 advance(Duration::from_secs(23 * 60 * 60 + 1)).await;
...
//                     let common = WebsocketCommon::new(vec![bad1, bad2, bad3], WebsocketMode::Pool(3), 0, None, None);
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let common = WebsocketCommon::new(vec![], WebsocketMode::Pool(2), 0, None, None);
//                     let connections = common.get_available_connections(false).await;
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let common = WebsocketCommon::new(vec![], WebsocketMode::Single, 0, None, None);
//                     let conn = common.get_connection(false).await.expect("should get connection");
...
//                     assert!(matches!(result, Err(crate::common::errors::WebsocketError::NotConnected)));
//                 });
//             }
...
//                     let mut st = conn.state.lock().await;
//                     st.pending_requests
//                         .insert("request_id".to_string(), PendingRequest { completion: req_tx });
//                 }
//                 let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                 let close_fut = common.close_connection_gracefully(tx.clone(), conn.clone());
...
//                     common.on_open("wss://example.com".into(), conn.clone(), None).await;
...
//                     assert_eq!(opened_url.lock().await.as_deref(), Some("wss://example.com"));
//                 });
//             }
...
//                     }
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     common.on_open("url".into(), conn.clone(), Some(old_tx.clone())).await;
...
//                     assert!(locked.iter().any(|e| matches!(e, WebsocketEvent::Message(m) if m == "msg")));
//                 });
//             }
...
//                     let evs = events.lock().await;
//                     assert!(evs.iter().any(|e| matches!(e, WebsocketEvent::Message(m) if m == "msg")));
...
//                     let expected_ua = build_user_agent();
...
//                     let url = format!("ws://{addr}");
//                     let res = WebsocketCommon::create_websocket(&url, None, Some(expected_ua)).await;
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let res = WebsocketCommon::create_websocket("not-a-valid-url", None, None).await;
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let res = WebsocketCommon::create_websocket("ws://127.0.0.1:1", None, None).await;
//                     assert!(matches!(res, Err(WebsocketError::Handshake(_))));
//                 });
//             }
//         }
...
//                         }
//                     });
//                     let conns: Vec<Arc<WebsocketConnection>> =
//                         (0..pool_size).map(|i| WebsocketConnection::new(format!("c{i}"))).collect();
//                     let common = WebsocketCommon::new(conns.clone(), WebsocketMode::Pool(pool_size), 0, None, None);
//                     let url = format!("ws://{addr}");
...
//                     let common = WebsocketCommon::new(vec![good, bad], WebsocketMode::Pool(2), 0, None, None);
//                     let url = format!("ws://{addr}");
...
//                     let common =
//                         WebsocketCommon::new(vec![c1.clone(), c2.clone()], WebsocketMode::Pool(2), 0, None, None);
//                     let url = format!("ws://{addr}");

//                     common.clone().init_connect(&url, false, None).await.unwrap();
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let url = format!("ws://{addr}");
//                     common.clone().init_connect(&url, false, Some(conn.clone())).await.unwrap();
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let url = format!("ws://{addr}");
//                     common.clone().init_connect(&url, false, Some(conn)).await.unwrap();
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let res = common.clone().init_connect("not-a-url", false, Some(conn.clone())).await;
//                     assert!(matches!(res, Err(WebsocketError::Handshake(_))));
//                 });
//             }
...
//                     }
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let res = common.clone().init_connect("ws://127.0.0.1:1", false, Some(conn.clone())).await;
...
//                     }
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let res = common.clone().init_connect("ws://127.0.0.1:1", true, Some(conn.clone())).await;
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let url = format!("ws://{addr}");
//                     let res = common.clone().init_connect(&url, true, Some(conn.clone())).await;
...
//                         assert!(st.renewal_pending, "renewal_pending must be true until on_open");
//                     }

//                     common.on_open(url.clone(), conn.clone(), None).await;

//                     let st = conn.state.lock().await;
//                     assert!(!st.renewal_pending, "renewal_pending should be cleared in on_open");
//                 });
//             }
...
//                     }
//                     let common =
//                         WebsocketCommon::new(vec![conn1.clone(), conn2.clone()], WebsocketMode::Pool(2), 0, None, None);
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let fut = common
//                         .send("msg".into(), Some("id".into()), true, Duration::from_secs(5), None)
//                         .await
//                         .unwrap()
//                         .unwrap();
//                     match rx.try_recv() {
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let err = common
//                         .send("msg".into(), None, true, Duration::from_secs(1), Some(conn.clone()))
//                         .await
//                         .unwrap_err();
//                     assert!(matches!(err, WebsocketError::NotConnected));
//                 });
...
//                     let common = WebsocketCommon::new(vec![conn.clone()], WebsocketMode::Single, 0, None, None);
//                     let err = common
//                         .send("msg".into(), Some("id".into()), true, Duration::from_secs(1), None)
//                         .await
//                         .unwrap_err();
//                     assert!(matches!(err, WebsocketError::NotConnected));
//                 });
...
//                     let sig_gen =
//                         SignatureGenerator::new(Some("api_secret".to_string()), None::<PrivateKey>, None::<String>);
...
//                         st.ws_write_tx = None;
//                     }
//                     let sig = SignatureGenerator::new(Some("api_secret".into()), None::<PrivateKey>, None::<String>);
...
//                 let sig_gen = SignatureGenerator::new(Some("api_secret".to_string()), None::<_>, None::<String>);
//                 let config = ConfigurationWebsocketApi {
//                     api_key: Some("api_key".to_string()),
//                     api_secret: Some("api_secret".to_string()),
//                     private_key: None,
...
//                     let st = conn.state.lock().await;
//                     assert!(!st.is_session_logged_on, "should remain logged‐off on failure");
//                 });
//             }
...
//                     {
//                         let mut st = conn.state.lock().await;
//                         st.pending_requests.insert("id1".to_string(), PendingRequest { completion: tx });
//                     }
...
//                     {
//                         let mut st = conn.state.lock().await;
//                         st.pending_requests.insert("bad".to_string(), PendingRequest { completion: tx });
//                     }
...
//                     {
//                         let mut st = conn.state.lock().await;
//                         st.pending_requests.insert("err".to_string(), PendingRequest { completion: tx });
//                     }
//                     let msg = json!({
//                         "id": "err",
//                         "status": 500,
...
//                         );
//                     }
//                     api.on_message(json!({ "id": "foo", "status": 200, "result": 1 }).to_string(), conn.clone())
//                         .await;
//                     let st = conn.state.lock().await;
...
//                     let conn1 = WebsocketConnection::new("c1");
//                     let conn2 = WebsocketConnection::new("c2");
//                     let api = WebsocketStreams::new(config.clone(), vec![conn1.clone(), conn2.clone()]);
...
//                     let state = conn.state.lock().await;
//                     let pending: Vec<String> = state.pending_subscriptions.iter().cloned().collect();
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let ws = create_websocket_streams(None, None);
//                     ws.clone().subscribe(vec!["s1".into(), "s2".into()], None).await;
//                     let map = ws.connection_streams.lock().await;
...
//                     {
//                         let mut state = conn.state.lock().await;
//                         state.stream_callbacks.insert("s2".to_string(), vec![Arc::new(|_: &Value| {})]);
//                     }
...
//                         state
//                             .stream_callbacks
//                             .insert("s1".to_string(), vec![Arc::new(|_: &Value| {}), Arc::new(|_: &Value| {})]);
//                     }
...
//                     }
//                     ws.unsubscribe(vec!["foo".into()], Some("bad-id".into())).await;
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let conns = vec![WebsocketConnection::new("c1"), WebsocketConnection::new("c2")];
//                     let config = ConfigurationWebsocketStreams {
//                         ws_url: Some("wss://example".to_string()),
//                         mode: WebsocketMode::Single,
//                         reconnect_delay: 100,
...
//                     assert_eq!(url, "wss://example/stream?streams=x/y/z&timeUnit=microsecond");
//                 });
//             }
//         }
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let ws = create_websocket_streams(None, None);
//                     let groups = ws.clone().handle_stream_assignment(vec!["s1".into(), "s2".into()]).await;
...
//                         }
//                     }
//                     assert_eq!(seen_streams, ["s1".to_string(), "s2".to_string()].iter().collect());
...
//                     let groups = ws.clone().handle_stream_assignment(vec!["s1".into(), "s3".into()]).await;
...
//                     let assigned = ws.handle_stream_assignment(vec!["s1".into(), "s2".into()]).await;
//                     assert_eq!(assigned.len(), 1);
...
//                     let conn = WebsocketConnection::new("c");
//                     let ws = create_websocket_streams(None, Some(vec![conn.clone()]));
//                     let _ = ws.handle_stream_assignment(vec!["s1".into(), "s2".into()]).await;
//                     let mixed = ws.handle_stream_assignment(vec!["s2".into(), "s3".into()]).await;
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let ws: Arc<WebsocketStreams> = create_websocket_streams(Some("ws://example.com"), None);
//                     let conn = &ws.common.connection_pool[0];
//                     let (tx, mut rx) = unbounded_channel();
//                     {
//                         let mut st = conn.state.lock().await;
//                         st.ws_write_tx = Some(tx);
//                     }
//                     ws.send_subscription_payload(conn, &vec!["s1".to_string()], Some("badid".to_string()));
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let ws: Arc<WebsocketStreams> = create_websocket_streams(Some("ws://unused"), None);
//                     let conn = &ws.common.connection_pool[0];
//                     let (tx, mut rx) = unbounded_channel();
//                     {
//                         let mut st = conn.state.lock().await;
...
//                     }
//                     ws.on_open("ws://example.com".to_string(), conn.clone()).await;
//                     let msg = rx.recv().await.expect("no subscription sent");
//                     if let Message::Text(txt) = msg {
//                         let v: Value = serde_json::from_str(&txt).unwrap();
//                         assert_eq!(v["method"], "SUBSCRIBE");
//                         let params = v["params"].as_array().unwrap();
//                         assert_eq!(params, &vec![Value::String("foo".into()), Value::String("bar".into())]);
//                     } else {
//                         panic!("unexpected message: {msg:?}");
//                     }
...
//                     {
//                         let mut st = conn.state.lock().await;
//                         st.stream_callbacks.entry("stream1".to_string()).or_default().push(
...
//                     }
//                     let url = ws.get_reconnect_url("default_url".into(), c0).await;
//                     let suffix = url.strip_prefix("ws://example.com/stream?streams=").unwrap();
...
//                 TOKIO_SHARED_RT.block_on(async {
//                     let mut ws: Arc<WebsocketStreams> = create_websocket_streams(Some("ws://example.com"), None);
//                     Arc::get_mut(&mut ws).unwrap().configuration.time_unit = Some(TimeUnit::Microsecond);
//                     let c0 = ws.common.connection_pool[0].clone();
//                     {
//                         let mut map = ws.connection_streams.lock().await;
...
//                     }
//                     let url = ws.get_reconnect_url("default_url".into(), c0).await;
//                     assert_eq!(url, "ws://example.com/stream?streams=x&timeUnit=microsecond");
//                 });
//             }
//         }
//     }
...
//                     {
//                         let mut state = conn.state.lock().await;
//                         state.stream_callbacks.insert(stream_name.clone(), Vec::new());
//                     }
...
//                     assert!(guard.is_none());
//                     let state = conn.state.lock().await;
//                     assert!(state.stream_callbacks.get(&stream_name).is_none_or(std::vec::Vec::is_empty));
//                 });
//             }
...
//                     {
//                         let stream_callbacks = ws_base.stream_callbacks.lock().await;
//                         let callbacks = stream_callbacks.get(&identifier).expect("Entry for 'id2' should exist");
//                         assert_eq!(callbacks.len(), 1);
//                     }
...
//                     {
//                         let stream_callbacks = ws_base.stream_callbacks.lock().await;
//                         let callbacks = stream_callbacks.get(&identifier).expect("Entry for 'id2' should still exist");
//                         assert!(callbacks.is_empty());
//                     }
//                 });
//             }
...
//             assert!(matches!(reason, WebsocketConnectionFailureReason::ConnectionReset));
...
//             let error = TungsteniteError::Io(io_error);
//             let reason = WebsocketConnectionFailureReason::from_tungstenite_error(&error);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::NetworkInterruption));
//             assert!(reason.should_reconnect());
...
//             let error = TungsteniteError::Io(io_error);
//             let reason = WebsocketConnectionFailureReason::from_tungstenite_error(&error);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::StreamEnded));
//             assert!(reason.should_reconnect());
...
//             let error = TungsteniteError::Io(io_error);
//             let reason = WebsocketConnectionFailureReason::from_tungstenite_error(&error);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::AuthenticationFailure));
//             assert!(!reason.should_reconnect());
...
//             let reason = WebsocketConnectionFailureReason::from_tungstenite_error(&error);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::ProtocolViolation));
//             assert!(!reason.should_reconnect());
...
//             assert!(matches!(reason, WebsocketConnectionFailureReason::NormalClose));
//             assert!(!reason.should_reconnect());

//             // Going away (server restart) -> ServerTemporaryError
//             let reason = WebsocketConnectionFailureReason::from_close_code(1001, false);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::ServerTemporaryError));
//             assert!(reason.should_reconnect());
...
//             assert!(matches!(reason, WebsocketConnectionFailureReason::UnexpectedClose));
//             assert!(reason.should_reconnect());

//             // Policy violation -> PermanentServerError
//             let reason = WebsocketConnectionFailureReason::from_close_code(1008, false);
//             assert!(matches!(reason, WebsocketConnectionFailureReason::PermanentServerError));
//             assert!(!reason.should_reconnect());
...
//             assert!(matches!(reason, WebsocketConnectionFailureReason::ConfigurationError));
//             assert!(!reason.should_reconnect());
...
//             assert!(matches!(reason, WebsocketConnectionFailureReason::UserInitiatedClose));
//             assert!(!reason.should_reconnect());
...
//             assert!(matches!(cloned, WebsocketConnectionFailureReason::NetworkInterruption));
